...
def build_project_registry(
    ...
    for project in projects:
        ...
        # BUG-4 FIX: Only add notebooklm when it's actually available
        if os.environ.get("NOTEBOOKLM_AVAILABLE", "").lower() in ("true", "1", "yes"):
...
def resolve_sources(workspace_root: Path, args: argparse.Namespace) -> SourceBundle:
    ...
    # BUG-1 FIX: Output MUST NOT overwrite input.
    # Input: projects.json (manual base, never auto-overwritten)
    # Output: data/projects_generated.json (safe to regenerate)
    default_out_projects = data_local / "projects_generated.json"
    ...
    return SourceBundle(
        ...
        out_projects_json=Path(args.out_projects_json) if args.out_projects_json else default_out_projects,
...
def build_dataset(bundle: SourceBundle) -> dict[str, Any]:
    ...
    for row in org_rows:
        ...
        # BUG-3 FIX: Normalize paths before dedup to avoid duplicates like
        # "deploy_russia.md" vs "Deploy_Russia.md" or mixed slashes
        seen_norm_paths: set[str] = set()
        deduped_workflow_paths: list[str] = []
        for p in related_workflow_paths:
            if not is_specific_path(p):
                continue
            norm = normalize_path(p)
            if norm not in seen_norm_paths:
                seen_norm_paths.add(norm)
                deduped_workflow_paths.append(p)
        related_workflow_paths = deduped_workflow_paths
        ...
        # BUG-2 FIX: Deduplicate notes segments to prevent accumulation
        # across repeated sync runs (e.g. tgaggregator notes ×39)
        raw_notes = [str(manual.get("notes", "")).strip(), str(row.get("notes", "")).strip()]
        note_segments = []
