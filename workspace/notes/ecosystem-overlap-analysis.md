...
## Post-Audit Quick Wins (2026-03-19)

- `projects.json` теперь трактуется как generated view, а ручные overrides переехали в `projects_manual_base.json`.
- `NotebookLM` выключен по умолчанию в launch contracts и включается только через `NOTEBOOKLM_AVAILABLE=true`.
- Root `AGENTS.md` теперь содержит явный write-back protocol: агент обязан возвращать долговременные результаты в vault/source-of-truth слой до очередного sync.
